...
class R2BSpec extends FlatSpec with Matchers with SimpleFixture {
  ...
  "Project" should "match" in {
    ...
    val backlogProject = backlog.getProject(config.backlogConfig.projectKey)
  ...
  "Group" should "match" in {
    ...
    val memberShips   = redmine.getMembershipManager.getMemberships(config.redmineConfig.projectKey).asScala
  ...
  "Project user" should "match" in {
    val backlogUsers = backlog.getProjectUsers(config.backlogConfig.projectKey).asScala
    val memberShips  = redmine.getMembershipManager.getMemberships(config.redmineConfig.projectKey).asScala
  ...
  "Version" should "match" in {
    val backlogVersions = backlog.getVersions(config.backlogConfig.projectKey).asScala
    val redmineProject  = redmine.getProjectManager.getProjectByKey(config.redmineConfig.projectKey)
  ...
  "Tracker" should "match" in {
    val backlogIssueTypes = backlog.getIssueTypes(config.backlogConfig.projectKey).asScala
  ...
  val backlogWikis = backlog.getWikis(config.backlogConfig.projectKey).asScala.map(backlogWiki => backlog.getWiki(backlogWiki.getId))
  val redmineWikis = redmine.getWikiManager.getWikiPagesByProject(config.redmineConfig.projectKey).asScala
  redmineWikis.foreach(redmineWiki =>
    "Wiki" should s"match: ${redmineWiki.getTitle}" in {
      val redmineWikiPageDetail = redmine.getWikiManager.getWikiPageDetailByProjectAndTitle(config.redmineConfig.projectKey, redmineWiki.getTitle)
  ...
  private[this] def issues(count: Int, offset: Long) = {
    ...
    redmineIssues.foreach(redmineIssue =>
      "Issue" should s"match: ${redmineIssue.getId}" in {
