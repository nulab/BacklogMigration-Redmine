...
class MappingValidator(redmineMappings: Seq[MappingItem], backlogMappings: Seq[MappingItem], itemName: String, filePath: String) {
  ...
  private def redmineItemsExists(mappings: Seq[Mapping]): Seq[String] =
  ...
  private def redmineItemExists(mappingItem: MappingItem, mappings: Seq[Mapping]): Option[String] =
  ...
  private def itemsExists(mappings: Seq[Mapping]): Seq[String] =
  ...
  private def itemExists(mapping: Mapping): Option[String] =
  ...
  private def itemsRequired(mappings: Seq[Mapping]): Seq[String] =
    mappings.foldLeft(Seq.empty[String])((errors: Seq[String], mapping: Mapping) => {
  ...
  private def itemRequired(mapping: Mapping): Option[String] =
