...
//  * @author uchida
//  */
//class PropertyServiceImpl(projectId: Int,
...
//  }
//
//  override def optUser2(id: Int): Option[User] =
//    users.find(_.getId.intValue() == id)
//
...
//    (for { id <- optId } yield users.find(_.getId.intValue() == id.toInt).map(_.getLogin)).flatten
//
...
//    } yield
//      memberships
//        .filter(membership => Option(membership.getUser).nonEmpty)
//        .find(_.getUser.getId.intValue() == id.toInt)
//        .map(_.getUser.getFullName)).flatten
//  }
//
...
//    (for { id <- optId } yield trackers.find(_.getId.intValue() == id.toInt).map(_.getName)).flatten
//
...
//      id         <- optId
//    } yield categories.find(_.getId.intValue() == id.toInt).map(_.getName)).flatten
//  }
//
